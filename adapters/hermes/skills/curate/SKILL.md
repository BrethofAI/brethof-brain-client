...
## Your tools
- `brethofmind_save(title, content, project)` — save one durable fact. Write
...
- `brethofmind_search(query)` — check first whether memory already holds it;
  if a result already says the same thing, saving again is noise.
- `brethofmind_delete(project, record_id)` — remove a saved memory the user
