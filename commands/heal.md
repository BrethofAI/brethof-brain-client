...
2. **Dedupe + contradictions** — for each candidate pair, `get_record 'a, b'`
   and judge:
...
3. **State dashboard** — for each `state:<area>` row, check the area's recent
   records (`recent_records` / `list_brain`): does the status line still
...
6. **Graph spot-check** — `get_record` a few linked records; fix or drop broken
